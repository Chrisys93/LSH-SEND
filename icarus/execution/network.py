    ...
    def most_proc_usage(self, list_len):
        ...
        while len(nodes) < list_len:
            ...
            for n in self.model.busy_proc:
                ...
                node_proc = self.model.avg_CPU_perc[n] + self.model.update_CPU_perc[n]
    ...
    def most_CPU_usage(self, exclude):
        ...
        for n in self.model.busy_proc:
            ...
            if (len(exclude) > 0 and all(h in exclude for h in self.model.all_node_h_spaces[n])) or len(self.model.all_node_h_spaces[n]) == 0:
                continue
            node_proc = self.model.avg_CPU_perc[n] + self.model.update_CPU_perc[n]
            if node_proc >= max_node_proc and node_proc > 0 and type(n) is int and len(self.model.busy_proc[n].keys()) > 0:
                max_node_proc = node_proc
                max_node = n
...
class NetworkModel(object):
    ...
    def __init__(self, topology, cache_policy, repo_policy, sched_policy, n_services, rate, seed=0, shortest_path=None):
        ...
        service_time_min = 0.03  # used to be 0.10 # used to be 0.001, 0.03 for Multi, 0.01 for MNIST
        service_time_max = 0.08 # used to be 0.10  # used to be 0.1, 0.08 for Multi, 0.015 for MNIST
        # delay_min = 0.005
...
class NetworkController(object):
    ...
    def reset_update_CPU_perc(self, node_list):
        for n in node_list:
            self.model.update_CPU_perc[n] = 0
