...
__all__ = ['uniform_content_placement', 'uniform_repo_content_placement',
           ...
           'dataset_random_repo_bucket_placement']
...
def apply_hash_reuse_placement(placement, topology):
    """Apply a placement to a topology
    Parameters
    ----------
    placement : dict of sets
    ...
    topology : Topology
        The topology
    """
    for node, hashes in placement.items():
        topology.nodes[node]['stack'][1].update(hash_reuse = hashes)
...
def dataset_repo_bucket_placement(topology, contents, freshness_per, shelf_life, msg_size, topics_weights,
                                 ...
                                 max_label_nos, seed=None, hash_reuse_file=None):
    """Places content objects to source nodes randomly according to the weight
    of the source node.
    TODO: This should be modified, or another one created, to include content
        placement parameters, like the freshness periods, shelf-lives, topics/types
    ...
    if hash_reuse_file:
        hash_reuse = dict()
        hash_reuse_placement = dict()
        with open(os.getcwd() + hash_reuse_file, 'r') as f:
            data = csv.reader(f)
            for row in data:  # row
                index = [float(s) for s in row[1].split() if s.isdigit()]
                reused = [float(s) for s in row[2].split() if s.isdigit()]
                reuse = float(reused[0]/index[0])
                hash_reuse[row[0]] = reuse
                for node in content_h_space:
                    if row[0] in content_h_space[node]:
                        if node not in hash_reuse_placement:
                            hash_reuse_placement[node] = dict()
                        hash_reuse_placement[node][row[0]] = reuse

        apply_hash_reuse_placement(hash_reuse_placement, topology)

    apply_content_placement(content_placement, topology)
    topology.placed_data = placed_data


@register_content_placement('DATASET_BUCKET_RAND_REPO_HASH')
def dataset_random_repo_bucket_placement(topology, contents, freshness_per, shelf_life, msg_size, topics_weights,
                                 types_weights, max_replications, num_of_repos, service_weights,
                                 max_label_nos, seed=None, hash_reuse_file=None):
    """Places content objects to source nodes randomly according to the weight
    of the source node.
    TODO: This should be modified, or another one created, to include content
    ...
    h_spaces = []
    for d in placed_data:
        rand = random.randint(0, num_of_repos - 1)
        if rand not in content_h_space:
            content_h_space[rand] = Counter()
        content_h_space[rand].update(placed_data[d]['h_space'])
    for n in content_h_space:
