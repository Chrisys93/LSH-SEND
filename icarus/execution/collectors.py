...
class RepoStatsOutputLatencyCollector(DataCollector):
    ...
    def results(self):
        ...
        if self.view.model.strategy == 'HYBRID':
            ...
            repo_incoming_BW = open("hash_proc_incoming" + str(self.rate) + ".txt", 'a')
            r_replicas = open("hash_proc_r_replicas" + str(self.rate) + ".txt", 'a')
            ...
            edge_proc = open("hash_proc_edge_proc" + str(self.rate) + ".txt", 'a')
            reuse_hits = open("hash_proc_reuse_hits" + str(self.rate) + ".txt", 'a')
            in_flight = open("hash_proc_in_flight" + str(self.rate) + ".txt", 'a')
            overall_delay_averages = open("hash_proc_overall_delay_avg" + str(self.rate) + ".txt", 'a')
            reuse_delay_averages = open("hash_proc_reuse_delay_avg" + str(self.rate) + ".txt", 'a')
            edge_delay_averages = open("hash_proc_edge_delay_avg" + str(self.rate) + ".txt", 'a')
            cloud_delay_averages = open("hash_proc_cloud_delay_avg" + str(self.rate) + ".txt", 'a')
            cpu_load_averages = open("hash_proc_cpu_load_avg" + str(self.rate) + ".txt", 'a')
            missed_node_reqs = open("hash_proc_missed_node_reqs" + str(self.rate) + ".txt", 'a')
        ...
        if self.view.model.strategy == 'HASH_PROC_REPO_APP':
            ...
            if self.view.model.split_buckets:
                for bucket in self.view.model.split_buckets:
                    for i in range(self.view.model.split_buckets[bucket]-1):
                        if bucket not in self.allocated_buckets:
                            self.allocated_buckets.append(bucket+"_"+str(i))
                for bucket in self.allocated_buckets:
                    if bucket not in self.requested_buckets:
                        self.requested_buckets.append(bucket)
            ...
            for n in self.view.model.orchestration_proc_workload:
                ...
                orch_bucket_count[n] += self.view.model.orchestration_proc_workload[n]
            ...
            for edr in range(len(self.view.model.source_node)):
                if edr in self.view.model.end_node_req_speed:
